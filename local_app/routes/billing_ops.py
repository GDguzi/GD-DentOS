...
def create_billing_ops_router(db_path):
    ...
    def get_bill(bill_id: str):
        """收费弹框：账单 + 逐项明细(单价/数量/总价/减免/费用分类)，供收款弹框展示。"""
        require_perm("billing.view")   # 账单明细含金额,按 billing.view 守卫
        with connect(db_path) as conn:
            ...
            # 导入单折扣在 source_json.DiscountFee(字符串带千分位逗号),
            # 复用欠费清单同款 SQL 口径解析;本地自建单该值恒 0(折扣已并入 total_fee)。
            import_discount = float(conn.execute(
        ...
        for it in irows:
            ...
            items.append({
                ...
                "refunded_fee": round(it["refunded_fee"] or 0, 2),   # 退费弹窗已退/可退列
            })
        ...
        return {
            ...
            # remaining 与欠费清单 bill_net_arrears_sql 同式(总-外部系统折扣-已收),打折同步单不再显示虚欠
            "remaining": round(total - import_discount - paid, 2),
            # ①:退款不再冲减 total/paid,已退与可退从退款流水派生,供退费弹窗封顶
            "refunded_fee": refunded, "refundable": round(paid - refunded, 2),
            "discount": round(discount or import_discount, 2), "items": items,
        }
    ...
    def _parse_amount(raw):
        ...
        if not math.isfinite(amt):   # 挡 nan/inf
            raise HTTPException(status_code=400, detail="收款金额无效")
    ...
    def confirm_payment(bill_id: str, payload: dict):
        ...
        payload = payload or {}
        rid, phash = _request_key(payload, bill_id, "pay")   # 幂等键必填(含 bill_id/动作)
        cashier = str(payload.get("cashier") or "").strip()  # 收银员(本地无登录，手填/记住上次)
        ...
        for m in raw_methods:
            ...
            if not method:   # 空方式=账目对不上现金/微信桶,任何一行为空整单拒(改钱路径禁止兜底)
        ...
        with connect(db_path) as conn:
            ...
            replay = _idempotent_replay(conn, rid, phash)   # 同号同载荷=重放首次结果,不落第二笔
    ...
    def refund_bill(bill_id: str, payload: dict):
        ...
        ①:退款只追加负数 payment 流水(payment_record_type=refund),落退款日冲减实收报表；
        bills.total_fee/paid_fee 永不冲减(应收/已收史实不可变,历史报表冻结),退满仅推进 state=refunded。
        可退额度 = paid_fee − Σ已退流水,事务内派生。
        红线：绝不退 导入单(非 local-bill- 前缀)。"""
        require_perm("billing_refund")   # P1-13：退费仅 admin
        payload = payload or {}
        rid, phash = _request_key(payload, bill_id, "refund")   # 幂等键必填(含 bill_id/动作)
        ...
        with connect(db_path) as conn:
            ...
            replay = _idempotent_replay(conn, rid, phash)   # 同号同载荷=重放,不再退第二笔
            if replay is not None:
                return replay
            bill = conn.execute(
                "select bill_id, patient_identity, total_fee, paid_fee, state from bills where bill_id = ?",
            ...
            if not bill_id.startswith("local-bill-"):
                raise HTTPException(status_code=409, detail="仅本地账单可退费，不可退 导入单")
            if bill["state"] != "paid":
            ...
            # ①:可退额度从退款流水派生(paid_fee 不再被退款冲减)。refund 流水仅本地退款写入。
            ...
            if isinstance(items_in, list) and items_in:
                ...
                # 可退额度=总价-累计已退(部分退过的项目剩余额度仍可继续按项退,退满才出局)
                valid_amt = {r["treatment_item_id"]: round((r["total_fee"] or 0) - (r["refunded_fee"] or 0), 2)
                    ...
                    (bill_id,)).fetchall()}
                refund_item_ids = []
                # per_iid 按 iid 累计,挡"同请求同item拆多行合计超额"
                for it in items_in:
                    iid = str((it or {}).get("treatment_item_id") or "")
                    # 每个提交项先严格校验属本单且未退(传错项目/已退项目都挡)
            ...
            # 三种退款路径统一在落库时确定项目归属 refund_items——按处置=用户逐项指定(精确);
            ...
            if per_iid:
                ...
                refund_items = []
                # 整数分分摊——逐项向下取整,余数一分一分补给"占比小数部分最大"的项,
            ...
            # ①(替代旧)：total_fee/paid_fee 一律不动——欠款式 total−优惠−paid 对已结清单
            ...
            conn.execute(
                ...
                (
                    ...
                    # 下游(today-collection by_method、患者缴费明细)读 $.pay_method,
                    # 退费此前只写 refund_method→退费金额取不到方式归入'未填',无法冲减真实收款方式。补 pay_method 键。
                    # refund_items=本笔退款的项目归属(流水自带,分类报表精确记类别)
            ...
            if refund_items:   # 三路径统一按归属累计 refunded_fee,退满(容差)才标已退
                conn.executemany(
