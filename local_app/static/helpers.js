...
// #541 诊所业务"今天/现在"一律北京时间(UTC+8,无夏令时)：宿主机/浏览器系统时区可能漂
...
// === SaaS 病历科目结构化值解析 ===
// 官方 SaaS 把 检查/诊断/治疗/医嘱 等科目存成 {"item":[{"RT":"4","RT_new":[{"tooth":"4",...}],"Text":"..."}]}。
// 解析为 [{teeth:["14"], text:"..."}]；不是该形态返回 null；item 为空返回 []（调用方按"无内容"跳过）。
function parseSaasSubject(value) {
  let parsed = value;
  ...
  if (Array.isArray(parsed)) return parseSaasSubjectArray(parsed);
  if (!parsed || typeof parsed !== "object" || !Array.isArray(parsed.item)) {
    return null;
  }
  // ⚠️牙位左右铁律：SaaS 象限键按【屏幕格子位置(viewer)】命名,屏幕右=患者左,故须左右翻转成 FDI(患者解剖侧)：
  // RT(屏右上)=患者左上=2, LT(屏左上)=患者右上=1, RB(屏右下)=患者左下=3, LB(屏左下)=患者右下=4。
  // 铁证:实际病历复核确认——主诉"左下大牙疼痛"的痛牙,其拔除计划在 SaaS 里存在 rb→必须落患者左下(FDI3)。改前务必先看此注释。
...
function parseSaasSubjectArray(arr) {
  // ⚠️同 parseSaasSubject：SaaS 小写象限键也是屏幕位(viewer),屏右=患者左,翻转成 FDI(患者解剖侧)。
  const QUAD = {rt: "2", lt: "1", lb: "4", rb: "3"};
...
// SaaS 结构化值转可读文本："14 缝线存，牙龈未见异常；…"；非该形态返回 null
function saasSubjectToText(value) {
...
// SaaS 病历模板科目存为 XML 富文本(<?xml..><Document><body><div><span>文本</span></div>..)。
// 转成可读纯文本：去声明/标签、div 边界换行、反转义实体。非 XML 字符串原样返回。
function saasXmlToText(value) {
...
// 回访状态码 → 可读文案(扫荡#400)：原始 "0"/空 直接显示成裸「0」很费解,统一映射。
...
function toothCrossHtml(teeth) {
  ...
  for (const code of teeth || []) {
    ...
    // #546:格子里只显示位号(如 16→"6")会丢象限信息(是右上还是左上),
  ...
  // #373:补完整 FDI 码到 title/aria-label，便于无障碍/自动化(hover 即见 "16")。
...
// #548:应用内输入弹窗，接口对齐 window.prompt(message, defaultValue) —— resolve 输入的字符串，
