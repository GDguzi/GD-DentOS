// #321/#362 回归:今日工作台左入口点击应在右侧壳内出简表,不跳管理页;高亮只亮当前一个。
...
test("点入口不再跳管理页:openTodaySaasEntry 不调 switchWorkspaceView", () => {
  const body = fnBody("openTodaySaasEntry");
  assert.ok(!/switchWorkspaceView/.test(body),
    "入口点击应在右侧壳出简表,跳转语义只属于 openTodaySaasEntryFull");
});

test("高亮只亮当前一个:先清全部 .today-saas-entry 的 active 再加到 button", () => {
  const body = fnBody("openTodaySaasEntry");
  assert.match(body, /querySelectorAll\(["']\.today-saas-entry["']\)[\s\S]*remove\(["']active["']\)/,
    "应先清掉所有入口的 active");
  assert.match(body, /button\.classList\.add\(["']active["']\)/,
    "再把 active 加到被点的入口");
});

test("简表渲染进右侧壳 #todaySaasMain", () => {
  const body = fnBody("openTodaySaasEntry");
  assert.match(body, /getElementById\(["']todaySaasMain["']\)/);
});

test("管理页查看全部仍保留跳转能力:openTodaySaasEntryFull 调 switchWorkspaceView", () => {
  const body = fnBody("openTodaySaasEntryFull");
  assert.match(body, /switchWorkspaceView/);
});

test("入口身份用 data-today-entry,不再用 data-today-action-view(避免同 view 双高亮)", () => {
  const list = fnBody("todaySaasEntryList");
