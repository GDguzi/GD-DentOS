// #541 回归:诊所业务"今天/现在"必须按北京时间(UTC+8),不受机器/浏览器系统时区影响。
...
// 跑：node --test test/web/test_bj_date_541.mjs
...
test("static/*.js 不得出现浏览器时区口径的业务日期取法", () => {
  ...
  for (const f of readdirSync(staticDir).filter(x => x.endsWith(".js"))) {
    ...
    for (const pat of banned) {
      assert.ok(!src.includes(pat), `${f} 含被禁用法 ${pat}(#541:业务日期须走 localDateValue()/bjToday()/bjNowStr())`);
    }
  }
});
